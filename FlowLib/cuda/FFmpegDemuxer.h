...
inline bool check(int e, int iLine, const char *szFile) {
    if (e < 0) {
        CV_LOG_ERROR(NULL, "General error " << e << " at line " << iLine << " in file " << szFile);
        return false;
    }
    return true;
}
    ...
    */
    FFmpegDemuxer(AVFormatContext *fmtc, int64_t timeScale = 1000 /*Hz*/) : fmtc(fmtc) {
        ...
        if (bMp4H264) {
            ...
            if (!bsf) {
                CV_LOG_ERROR(NULL, "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "av_bsf_get_by_name() failed");
                return;
            }
...
public:
    ...
    bool SeekFrame(int64_t frameNr) {
        ...
        if (ret < 0)
        {
            CV_LOG_ERROR(NULL, "FFmpeg seek failed");
            return false;
        }
            ...
            *pnVideoBytes = pktFiltered.size;
            ...
            if (bMp4MPEG4 && (frameCount == 0)) {
                ...
                if (extraDataSize > 0) {
                    ...
                    if (!pDataWithHeader) {
                        CV_LOG_ERROR(NULL, "FFmpeg error: " << __FILE__ << " " << __LINE__);
                        return false;
                    }
