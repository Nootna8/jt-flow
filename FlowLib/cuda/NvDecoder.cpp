...
int NvDecoder::HandlePictureDisplay(CUVIDPARSERDISPINFO *pDispInfo) {
    ...
    cv::cuda::GpuMat pDecodedFrame(m_nSurfaceHeight, m_nSurfaceWidth, CV_8UC1);
    videoDecPostProcessFrameGREY(dpSrcFrame, nSrcPitch, (CUdeviceptr)pDecodedFrame.ptr<uint>(), pDecodedFrame.step, m_nSurfaceWidth, m_nSurfaceHeight, m_cuvidStream);
...
int64_t NvDecoder::PeekTimestamp()
{
    if (frameQueue.size() == 0)
        return 0;

    return frameQueue.back().timeStamp;
}
